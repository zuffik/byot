...
export const ProcessActionCreator = <
  ...
  QP,
  RP = any,
  P extends AsynchronousActionResponse<QP, RP> = AsynchronousActionResponse<
    QP,
    RP
  >
>({
...
}: ProcessActionCreatorOptions = {}) => (Target: {
  new (): ProcessAction<S, QP, RP, P>;
}) => {
  ...
  const actionName =
    process.env.NODE_ENV === 'production' ? (++id).toString() : Target.name;
  ...
  Actions.reduxPack[group][actionName] = {
    entryAction,
    secondaryAction: (secondaryAction as any) as ActionCreator<
      AsynchronousActionResponse<any, any>
    >,
    reducer,
  };
};
